...
## Connected cohort release — 8 September 2026

Risk Calculator, Risk Over Time and What-If now use the selected patient rather than the earlier standalone example. The directory offers 5,000 generated patients through a paginated index and 50-patient shards. One generated patient is loaded into the active workspace at a time; edited generated records are stored under patient-specific local-storage keys. Existing hand-authored scenarios and their local edits are retained. Earlier unassigned standalone risk examples are not assigned to a patient automatically.

All 53 baseline attributes plus PATIENT_ID are available in a read-only provenance panel. New observations are appended to My Health; baseline fields remain unchanged. Lipid and glucose unit conversion is shared by the calculator and measurement chart. Missing predictors and unknown history never become silent defaults. Saved assessment snapshots keep the patient ID, calculation version, profile, source observations and review status. Scenarios keep their own baseline and are excluded from recorded-risk trajectories. A saved assessment/scenario can become a care-plan discussion task, not an achieved measurement or automatic treatment target.

Research Lab displays the separate 3,000/1,000/1,000 synthetic training/validation/test experiment. The generated endpoint, assumptions, preprocessing, model selection policy and reproducible script are documented in `research/README.md`. The experiment is not clinical validation and its outputs are never used by the patient calculator. The original stakeholder documents predate this release and retain their original text with a release-context notice.

Validation: 22 automated tests, TypeScript checking, static export and data-bundle checksum checks. Tests cover cohort integrity, patient splitting, unit conversion, immutable records, missing inputs, eligibility, scenario separation and the earlier clinical helper checks. No claim of complete browser, load, security or clinical validation is made.
